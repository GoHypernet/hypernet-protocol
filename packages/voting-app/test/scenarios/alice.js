...
contract('SmartAccount', function(accounts) {
    ...
    describe("Scenario: Alice has a current account with her bank, but wants to open an account for her business.", function() {
        ...
        it("Should create an account for Alice so that she can create KYC attributes", function() {
            ...
            .then(function(data) {
                ...
                return data.setEncryptionPublicKey(encryptionKey, {from: alice.address})
                    ...
                    SmartAccount.at(alice.account).then(function(account){
        ...
        it("Should create an account for Alice's Retail Bank so they can endorse her KYC checks", function() {
            return SmartAccount.new({from: retailBank.address})
            .then(function(data) {
        ...
        it("Should create an account for Alice's Commercial Bank so they can verify the KYC checks", function() {
            return SmartAccount.new({from: commercialBank.address})
            .then(function(data) {
                commercialBank.account = data.address;
                assert.isOk(data, "Alice's Commercial Bank Account failed to be created");
            });
        });

        it("Should allow Alice to create a new KYC attribute", function() {
            return SmartAccount.at(alice.account).addAttribute(kycAttributeHash, {from: alice.address})
            .then(function(data) {
        ...
        it("Should allow Alice's Retail Bank to endorse Alice's KYC attribute", function() {
            return SmartAccount.at(alice.account).addEndorsement(kycAttributeHash, endorsementHash, {from: retailBank.address})
            .then(function(data) {
        ...
        it("Should allow Alice to accept her Retail Bank's endorsement for her attribute", function() {
            return SmartAccount.at(alice.account).acceptEndorsement(kycAttributeHash, endorsementHash, {from: alice.address})
            .then(function(data) {
        ...
        it("Should allow Alice's Bank to check that an endorsement exists for her KYC attribute; verifying her KYC checks", function() {
            return SmartAccount.at(alice.account).checkEndorsementExists.call(kycAttributeHash, endorsementHash, {from: commercialBank.address})
            .then(function(data) {
