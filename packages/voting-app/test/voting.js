...
contract('Voting App', ([root, holder1, holder2, holder20, holder29, holder51, nonHolder]) => {
  ...
  const errors = makeErrorMappingProxy({
    ...
    // Voting errors
    HYPERNETLABS_VOTING_NO_VOTE: 'HYPERNETLABS_VOTING_NO_VOTE',
    HYPERNETLABS_VOTING_VOTE_ID_ZERO: 'HYPERNETLABS_VOTING_VOTE_ID_ZERO',
    HYPERNETLABS_VOTING_INIT_PCTS: 'HYPERNETLABS_VOTING_INIT_PCTS',
    HYPERNETLABS_VOTING_CHANGE_SUPPORT_PCTS: 'HYPERNETLABS_VOTING_CHANGE_SUPPORT_PCTS',
    HYPERNETLABS_VOTING_CHANGE_QUORUM_PCTS: 'HYPERNETLABS_VOTING_CHANGE_QUORUM_PCTS',
    HYPERNETLABS_VOTING_INIT_SUPPORT_TOO_BIG: 'HYPERNETLABS_VOTING_INIT_SUPPORT_TOO_BIG',
    HYPERNETLABS_VOTING_CHANGE_SUPP_TOO_BIG: 'HYPERNETLABS_VOTING_CHANGE_SUPP_TOO_BIG',
    HYPERNETLABS_VOTING_CAN_NOT_VOTE: 'HYPERNETLABS_VOTING_CAN_NOT_VOTE',
    HYPERNETLABS_VOTING_CAN_NOT_EXECUTE: 'HYPERNETLABS_VOTING_CAN_NOT_EXECUTE',
    HYPERNETLABS_VOTING_CAN_NOT_FORWARD: 'HYPERNETLABS_VOTING_CAN_NOT_FORWARD',
    HYPERNETLABS_VOTING_ORACLE_SENDER_MISSING: 'HYPERNETLABS_VOTING_ORACLE_SENDER_MISSING',
    HYPERNETLABS_VOTING_ORACLE_SENDER_TOO_BIG: 'HYPERNETLABS_VOTING_ORACLE_SENDER_TOO_BIG',
    HYPERNETLABS_VOTING_ORACLE_SENDER_ZERO: 'HYPERNETLABS_VOTING_ORACLE_SENDER_ZERO'
  })
  ...
  context('normal token supply, common tests', () => {
    ...
    it('fails changing required support lower than minimum acceptance quorum', async () => {
      await assertRevert(voting.changeSupportRequiredPct(minimumAcceptanceQuorum.sub(toBn(1))), errors.HYPERNETLABS_VOTING_CHANGE_SUPPORT_PCTS)
    })

    it('fails changing required support to 100% or more', async () => {
      await assertRevert(voting.changeSupportRequiredPct(pct16(101)), errors.HYPERNETLABS_VOTING_CHANGE_SUPP_TOO_BIG)
      await assertRevert(voting.changeSupportRequiredPct(pct16(100)), errors.HYPERNETLABS_VOTING_CHANGE_SUPP_TOO_BIG)
    })
    ...
    it('fails changing minimum acceptance quorum to greater than min support', async () => {
      await assertRevert(voting.changeMinAcceptQuorumPct(neededSupport.add(toBn(1))), errors.HYPERNETLABS_VOTING_CHANGE_QUORUM_PCTS)
    })
    ...
    context(`normal token supply, ${decimals} decimals`, () => {
      ...
      context('creating vote', () => {
        ...
        it('fails getting a vote out of bounds', async () => {
          await assertRevert(voting.getVote(voteId + 1), errors.HYPERNETLABS_VOTING_NO_VOTE)
        })

        it('fails getting a vote with id 0', async () => {
          await assertRevert(voting.getVote(0), errors.HYPERNETLABS_VOTING_VOTE_ID_ZERO)
        })
        ...
        it('throws when voter stake becomes 0 after vote start', async () => {
          ...
          await assertRevert(voting.vote(voteId, true, { from: holder29 }), errors.HYPERNETLABS_VOTING_CAN_NOT_VOTE)
        })

        it('throws when non-holder votes', async () => {
          await assertRevert(voting.vote(voteId, true, { from: nonHolder }), errors.HYPERNETLABS_VOTING_CAN_NOT_VOTE)
        })
        ...
        it('throws when voting before start block', async () => {
          ...
          await assertRevert(voting.vote(newVoteId, true, { from: holder29 }), errors.HYPERNETLABS_VOTING_CAN_NOT_VOTE)
        })
        ...
        it('cannot execute vote if not enough quorum met', async () => {
          await voting.vote(voteId, true, { from: holder20 })
          await voting.mockAdvanceBlocks(durationBlocks + executionDelayBlocks)
          await assertRevert(voting.executeVote(voteId), errors.HYPERNETLABS_VOTING_CAN_NOT_EXECUTE)
        })
        ...
        it('voter can\'t change vote', async () => {
          await voting.vote(voteId, true, { from: holder29 })
          await assertRevert(voting.vote(voteId, false, { from: holder29 }), errors.HYPERNETLABS_VOTING_CAN_NOT_VOTE)
        })
        ...
        it('cannot execute unvoted vote before start block', async () => {
          ...
          assert(parseInt(newVoteStartBlock) > parseInt(currentBlock), 'new vote start block should be ahead of current block')
          await assertRevert(voting.executeVote(newVoteId), errors.HYPERNETLABS_VOTING_CAN_NOT_EXECUTE)
        })
        ...
        describe('ACLOracle canPerform()', () => {
          ...
          it('reverts when passed empty address array', async () => {
            await assertRevert(voting.canPerform(ANY_ADDR, ANY_ADDR, '0x', []), errors.HYPERNETLABS_VOTING_ORACLE_SENDER_MISSING)
          })

          it('reverts when passed uint is out of range', async () => {
            const outOfRangeAddressAsInt = toBn(2).pow(toBn(160))
            await assertRevert(voting.canPerform(ANY_ADDR, ANY_ADDR, '0x', [outOfRangeAddressAsInt]), errors.HYPERNETLABS_VOTING_ORACLE_SENDER_TOO_BIG)
          })

          it('reverts when passed 0 as address', async () => {
            await assertRevert(voting.canPerform(ANY_ADDR, ANY_ADDR, '0x', [0]), errors.HYPERNETLABS_VOTING_ORACLE_SENDER_ZERO)
          })
  ...
  context('wrong initializations', () => {
    ...
    it('fails if min acceptance quorum is greater than min support', async () => {
      ...
      await assertRevert(voting.initialize(token.address, neededSupport, minimumAcceptanceQuorum, durationBlocks, bufferBlocks, executionDelayBlocks), errors.HYPERNETLABS_VOTING_INIT_PCTS)
    })

    it('fails if min support is 100% or more', async () => {
      const minimumAcceptanceQuorum = pct16(20)
      await assertRevert(voting.initialize(token.address, pct16(101), minimumAcceptanceQuorum, durationBlocks, bufferBlocks, executionDelayBlocks), errors.HYPERNETLABS_VOTING_INIT_SUPPORT_TOO_BIG)
      await assertRevert(voting.initialize(token.address, pct16(100), minimumAcceptanceQuorum, durationBlocks, bufferBlocks, executionDelayBlocks), errors.HYPERNETLABS_VOTING_INIT_SUPPORT_TOO_BIG)
    })
  })
  ...
  context('empty token', () => {
    ...
    it('prevents voting if token has no holder', async () => {
      ...
      await assertRevert(voting.vote(voteId, true, { from: holder1 }), errors.HYPERNETLABS_VOTING_CAN_NOT_VOTE)
    })
  })
  ...
  context('before init', () => {
    ...
    it('fails to forward actions before initialization', async () => {
      const action = { to: executionTarget.address, calldata: executionTarget.contract.methods.execute().encodeABI() }
      const script = encodeCallScript([action])
      await assertRevert(voting.forward(script, { from: holder51 }), errors.HYPERNETLABS_VOTING_CAN_NOT_FORWARD)
    })
  })
