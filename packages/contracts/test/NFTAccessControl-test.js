...
describe("NFT Access Control Testing", function () {
  ...
  it("Check Token Role Enforcement.", async function () {
    ...
    expect(
      await test["hasRole(bytes32,uint256)"](ADMINROLE, adminTokenId),
    ).to.equal(true);
    expect(await test["hasRole(bytes32,uint256)"](ADMINROLE, 2)).to.equal(
      false,
    );
    expect(
      await test["hasRole(bytes32,uint256)"](ADMINROLE, owner.address),
    ).to.equal(false);
  });
  ...
  it("Check Token Role Enforcement.", async function () {
    ...
    const ADMINROLE = test.DEFAULT_ADMIN_ROLE();
    expect(
      await test["hasRole(bytes32,address)"](ADMINROLE, addr1.address),
    ).to.equal(true);
    expect(
      await test["hasRole(bytes32,address)"](ADMINROLE, owner.address),
    ).to.equal(false);
