...
contract('Voting App', ([root, holder1, holder2, holder20, holder29, holder51, nonHolder]) => {
      ...
      context('creating vote', () => {
        ...
        describe('ACLOracle canPerform()', () => {
          ...
          it('reverts when passed empty address array', async () => {
            await assertRevert(voting.canPerform(ANY_ADDR, ANY_ADDR, '0x', []), errors.VOTING_ORACLE_SENDER_MISSING)
          })

          it('reverts when passed uint is out of range', async () => {
            const outOfRangeAddressAsInt = toBn(2).pow(toBn(160))
            await assertRevert(voting.canPerform(ANY_ADDR, ANY_ADDR, '0x', [outOfRangeAddressAsInt]), errors.VOTING_ORACLE_SENDER_TOO_BIG)
          })

          it('reverts when passed 0 as address', async () => {
            await assertRevert(voting.canPerform(ANY_ADDR, ANY_ADDR, '0x', [0]), errors.VOTING_ORACLE_SENDER_ZERO)
          })
  ...
  context('wrong initializations', () => {
    ...
    it('fails if min acceptance quorum is greater than min support', async () => {
      ...
      await assertRevert(voting.initialize(token.address, neededSupport, minimumAcceptanceQuorum, durationBlocks, bufferBlocks, executionDelayBlocks), errors.VOTING_INIT_PCTS)
    })

    it('fails if min support is 100% or more', async () => {
      const minimumAcceptanceQuorum = pct16(20)
      await assertRevert(voting.initialize(token.address, pct16(101), minimumAcceptanceQuorum, durationBlocks, bufferBlocks, executionDelayBlocks), errors.VOTING_INIT_SUPPORT_TOO_BIG)
      await assertRevert(voting.initialize(token.address, pct16(100), minimumAcceptanceQuorum, durationBlocks, bufferBlocks, executionDelayBlocks), errors.VOTING_INIT_SUPPORT_TOO_BIG)
    })
  })
