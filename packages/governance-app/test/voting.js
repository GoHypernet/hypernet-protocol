...
const getBlockNumber = require("@aragon/contract-test-helpers/blockNumber")(
  web3,
);
const { encodeCallScript } = require("@aragon/contract-test-helpers/evmScript");
const {
  makeErrorMappingProxy,
} = require("@aragon/contract-test-helpers/utils");
...
contract(
  ...
  ([root, holder1, holder2, holder20, holder29, holder51, nonHolder]) => {
    ...
    const errors = makeErrorMappingProxy({
      ...
      VOTING_VOTE_ID_ZERO: "VOTING_VOTE_ID_ZERO",
      ...
      VOTING_ORACLE_SENDER_MISSING: "VOTING_ORACLE_SENDER_MISSING",
      VOTING_ORACLE_SENDER_TOO_BIG: "VOTING_ORACLE_SENDER_TOO_BIG",
      VOTING_ORACLE_SENDER_ZERO: "VOTING_ORACLE_SENDER_ZERO",
    });
    ...
    context("normal token supply, common tests", () => {
      ...
      it("fails changing required support lower than minimum acceptance quorum", async () => {
        await assertRevert(
          voting.changeSupportRequiredPct(minimumAcceptanceQuorum.sub(toBn(1))),
      ...
      it("fails changing minimum acceptance quorum to greater than min support", async () => {
        await assertRevert(
          voting.changeMinAcceptQuorumPct(neededSupport.add(toBn(1))),
      ...
      it("can change vote buffer blocks", async () => {
        ...
        assert.equal(
          await voting.bufferBlocks(),
          expectedBufferBlocks,
          "should have changed buffer blocks",
        );
      });

      it("can change execution delay blocks", async () => {
        const expectedExecutionDelayBlocks = 50;
        const receipt = await voting.changeExecutionDelayBlocks(
          expectedExecutionDelayBlocks,
        );
        assertAmountOfEvents(receipt, "ChangeExecutionDelayBlocks");

        assert.equal(
          await voting.executionDelayBlocks(),
          expectedExecutionDelayBlocks,
          "should have changed execution delay blocks",
        );
      });
    });
      ...
      context(`normal token supply, ${decimals} decimals`, () => {
        ...
        it("execution scripts can execute multiple actions", async () => {
          ...
          const voteId = createdVoteId(
            await voting.newVote(script, "", true, { from: holder51 }),
          );
        ...
        it("execution script can be empty", async () => {
          await voting.newVote(encodeCallScript([]), "", true, {
            from: holder51,
          });
        });
        ...
        it("ACLOracle canPerform() returns true when no votes have been created", async () => {
          assert.isTrue(
            await voting.canPerform(holder1, ANY_ADDR, "0x", [holder1]),
          );
        });
        ...
        context("creating vote", () => {
          ...
          beforeEach(async () => {
            ...
            const receipt = await voting.newVote(script, "metadata", false, {
              from: holder51,
            });
          ...
          it("has correct state", async () => {
            ...
            assert.equal(
              ...
              "start block should be correct",
            );
            assert.equal(
              executionBlock.toString(),
              startBlock.toNumber() + executionDelayBlocks + durationBlocks,
              "execution block should be correct",
            );
          ...
          it("fails getting a vote with id 0", async () => {
            await assertRevert(voting.getVote(0), errors.VOTING_VOTE_ID_ZERO);
          });
          ...
          it("changing required support does not affect vote required support", async () => {
            ...
            await voting.mockAdvanceBlocks(
              durationBlocks + executionDelayBlocks,
            );
          ...
          it("changing delay blocks doesnt affect vote delay blocks", async () => {
            ...
            const expectedExecutionBlock =
              startBlock.toNumber() + executionDelayBlocks + durationBlocks;
            assert.equal(
              executionBlock.toString(),
              expectedExecutionBlock.toString(),
              "execution blocks in vote should not change",
            );
            await voting.executeVote(voteId); // exec doesn't fail
          });
          ...
          it("uses snapshot balance as vote weight when balance increases after vote start", async () => {
            ...
            assert.equal(
              yea.toString(),
              bigExp(29, decimals).toString(),
              "snapshot balance should have been added",
            );
            assert.equal(
              currentBalance.toString(),
              bigExp(30, decimals).toString(),
              "balance should be 30 at current block",
            );
          });

          it("uses current balance as vote weight when balance decreases after vote start", async () => {
            await token.transfer(nonHolder, bigExp(1, decimals), {
              from: holder29,
            });
            ...
            assert.equal(
              yea.toString(),
              bigExp(28, decimals).toString(),
              "current balance should have been added",
            );
            assert.equal(
              currentBalance.toString(),
              bigExp(28, decimals).toString(),
              "balance should be 28 at current block",
            );
          });
          ...
          it("throws when voter stake becomes 0 after vote start", async () => {
            ...
            await assertRevert(
              voting.vote(voteId, true, { from: holder29 }),
          ...
          it("throws when non-holder votes", async () => {
            await assertRevert(
              voting.vote(voteId, true, { from: nonHolder }),
              errors.VOTING_CAN_NOT_VOTE,
            );
          });
          ...
          it("throws when voting before start block", async () => {
            const newVoteId = createdVoteId(
              await voting.newVote(script, "metadata", false, {
                from: holder51,
              }),
            );
            const { startBlock: newVoteStartBlock } = await voting.getVote(
              newVoteId,
            );
            const currentBlock = await voting.getBlockNumberPublic();

            assert(
              parseInt(newVoteStartBlock) > parseInt(currentBlock),
              "new vote start block should be ahead of current block",
            );

            await assertRevert(
              voting.vote(newVoteId, true, { from: holder29 }),
              errors.VOTING_CAN_NOT_VOTE,
            );
          });
          ...
          it("cannot execute vote before execution block", async () => {
            ...
            await voting.vote(voteId, false, { from: holder20 });
            await voting.mockAdvanceBlocks(
              durationBlocks + executionDelayBlocks - blocksSinceVoteCreation,
            );
            await assertRevert(
              voting.executeVote(voteId),
              errors.VOTING_CAN_NOT_EXECUTE,
          ...
          it("voter can't change vote", async () => {
            await voting.vote(voteId, true, { from: holder29 });
            await assertRevert(
              voting.vote(voteId, false, { from: holder29 }),
              errors.VOTING_CAN_NOT_VOTE,
            );
          });
          ...
          it("cannot execute unvoted vote before start block", async () => {
            ...
            await assertRevert(
              voting.executeVote(newVoteId),
              errors.VOTING_CAN_NOT_EXECUTE,
            );
          });

          it("sets second vote start and snapshot block correctly when vote created before vote buffer elapsed", async () => {
            const { startBlock: currentVoteStartBlock } = await voting.getVote(
              voteId,
            );
            const expectedVoteStartBlock =
              parseInt(currentVoteStartBlock) + bufferBlocks;
            ...
            const {
              startBlock: newVoteStartBlock,
              snapshotBlock: newVoteSnapshotBlock,
            } = await voting.getVote(newVoteId);
          ...
          it("sets third vote start and snapshot block correctly when all created within vote buffer", async () => {
            const secondVoteId = createdVoteId(
              await voting.newVote(script, "metadata", false, {
                from: holder51,
              }),
            );
            const { startBlock: secondVoteStartBlock } = await voting.getVote(
              secondVoteId,
            );
            const expectedVoteStartBlock =
              parseInt(secondVoteStartBlock) + bufferBlocks;

            const thirdVoteId = createdVoteId(
              await voting.newVote(script, "metadata", false, {
                from: holder51,
              }),
            );
            ...
            const {
              startBlock: thirdVoteStartBlock,
              snapshotBlock: thirdVoteSnapshotBlock,
            } = await voting.getVote(thirdVoteId);
          ...
          it("sets second vote start and snapshot block correctly when vote created after vote buffer elapsed", async () => {
            const expectedVoteStartBlock =
              (await getBlockNumber()) + bufferBlocks + 1;
          ...
          it("last yea vote id for voter set to voteId of vote voted for", async () => {
            ...
            const actualLatestYeaVoteId = await voting.latestYeaVoteId(
              holder29,
            );
          ...
          describe("last yea voteId on second vote", () => {
            ...
            it("updates when voting on a second vote", async () => {
              ...
              assert.equal(
                actualLatestYeaVoteId.toString(),
                secondVoteId.toString(),
              );
              assert.notEqual(
                actualLatestYeaVoteId.toString(),
                voteId.toString(),
          ...
          const itChecksNoRecentPositiveVotesCorrectly = (
            functionCallingNoRecentPositiveVotes,
          ) => {
            it("returns true when not voted in any votes", async () => {
              assert.isTrue(
                await functionCallingNoRecentPositiveVotes(holder29),
              );
            });
            ...
            it("returns true when voted yea and vote finished, failed and execution delay passed", async () => {
              ...
              assert.isTrue(
                await functionCallingNoRecentPositiveVotes(holder20),
              );
            });
            ...
            it("returns false when voted yea and vote finished, failed and before execution delay passed", async () => {
              ...
              await voting.vote(voteId, false, { from: holder29 });
              await voting.mockAdvanceBlocks(
                durationBlocks + executionDelayBlocks - 4,
              );
            ...
            it("returns true when voted yea and vote not executed but execution period passed", async () => {
              await voting.vote(voteId, true, { from: holder29 });
              await voting.mockAdvanceBlocks(
                durationBlocks + executionDelayBlocks + executionPeriod,
              );
            ...
            it("returns false when voted yea and vote finished, not executed and before execution period passed", async () => {
              await voting.vote(voteId, true, { from: holder29 });
              await voting.mockAdvanceBlocks(
                durationBlocks + executionDelayBlocks + executionPeriod - 2,
              );
          ...
          describe("ACLOracle canPerform()", () => {
            it("reverts when passed empty address array", async () => {
              await assertRevert(
                voting.canPerform(ANY_ADDR, ANY_ADDR, "0x", []),
                errors.VOTING_ORACLE_SENDER_MISSING,
              );
            });
            ...
            it("reverts when passed uint is out of range", async () => {
              ...
              await assertRevert(
                voting.canPerform(ANY_ADDR, ANY_ADDR, "0x", [
                  outOfRangeAddressAsInt,
                ]),
                errors.VOTING_ORACLE_SENDER_TOO_BIG,
              );
            });

            it("reverts when passed 0 as address", async () => {
              await assertRevert(
                voting.canPerform(ANY_ADDR, ANY_ADDR, "0x", [0]),
                errors.VOTING_ORACLE_SENDER_ZERO,
              );
            });

            itChecksNoRecentPositiveVotesCorrectly(
              async (sender) =>
                await voting.canPerform(ANY_ADDR, ANY_ADDR, "0x", [sender]),
            );
          });
          ...
          describe("TokenManagerHook _transfer()", () => {
            itChecksNoRecentPositiveVotesCorrectly(
              async (sender) =>
                await voting.onTransfer.call(sender, ANY_ADDR, bigExp(1, 18)),
            );
          });
        });
      });
    ...
    context("empty token", () => {
      ...
      it("prevents voting if token has no holder", async () => {
        const voteId = createdVoteId(
          await voting.newVote(EMPTY_SCRIPT, "metadata", true),
        );
    ...
    context("before init", () => {
      it("fails creating a vote before initialization", async () => {
        await assertRevert(
          voting.newVote(encodeCallScript([]), "", true),
    ...
    context("isValuePct unit test", async () => {
      ...
      it("tests pct ~= 100", async () => {
        const result1 = await voting.isValuePct(
          10,
          10,
          pct16(100).sub(toBn(1)),
        );
