...
contract("SmartAccount", function (accounts) {
  ...
  describe("Encryption Public Key tests", function () {
    it("will set encryption public key as the owner", function () {
      return smartAccount
        .setEncryptionPublicKey(publicEncryptionKey, { from: owner })
    ...
    it("will confirm that the owner's encryption public key was set successfully", function () {
      return smartAccount.encryptionPublicKey
        .call({ from: owner })
        .then(function (response) {
          assert.equal(
            response,
            publicEncryptionKey,
            "encryptionPublicKey doesn't exist",
          );
        });
    });
    ...
    it("will update the encryption public key as the owner", function () {
      return smartAccount
        .setEncryptionPublicKey(newPublicEncryptionKey, { from: owner })
        .then(function (response) {
          var newEncryptKeyStatus = response.logs[0].args.status;
          assert.equal(
            3,
      ...
      return smartAccount.encryptionPublicKey
        ...
        .then(function (response) {
          assert.equal(
            response,
            newPublicEncryptionKey,
            "encryptionPublicKey has not been changed",
          );
        });
    });
    ...
    it("will not allow a third party to modify the owner's encryption public key", function () {
      return smartAccount
        .setEncryptionPublicKey(publicEncryptionKey, { from: thirdparty })
      ...
      return smartAccount.encryptionPublicKey
        ...
        .then(function (response) {
          assert.equal(
            response,
            newPublicEncryptionKey,
            "encryptionPublicKey has been changed",
          );
        });
    });
