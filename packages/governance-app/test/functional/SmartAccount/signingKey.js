...
contract("SmartAccount", function (accounts) {
  var smartAccount, owner, thirdparty, publicSigningKey, newPublicSigningKey;

  before(
    "Setup the smart Account and hydrate the required variables",
    function (done) {
      owner = accounts[0];
  ...
  describe("Signing Public Key tests", function () {
    it("will set signing public key as the owner", function () {
      return smartAccount
        .setSigningPublicKey(publicSigningKey, { from: owner })
    ...
    it("will confirm that the owner's signing public key was set successfully", function () {
      return smartAccount.signingPublicKey
        .call({ from: owner })
        .then(function (response) {
          assert.equal(
            response,
            publicSigningKey,
            "signingPublicKey doesn't exist",
          );
        });
    });
    ...
    it("will update the signing public key as the owner", function () {
      return smartAccount
        .setSigningPublicKey(newPublicSigningKey, { from: owner })
        .then(function (response) {
          var newSigningKeyStatus = response.logs[0].args.status;
          assert.equal(
            3,
      ...
      return smartAccount.signingPublicKey
        ...
        .then(function (response) {
          assert.equal(
            response,
            newPublicSigningKey,
            "signingPublicKey has not been changed",
          );
        });
    });
    ...
    it("will not allow a third party to modify the owner's signing public key", function () {
      return smartAccount
        .setSigningPublicKey(publicSigningKey, { from: thirdparty })
      ...
      return smartAccount.signingPublicKey
        ...
        .then(function (response) {
          assert.equal(
            response,
            newPublicSigningKey,
            "signingPublicKey has been changed",
          );
        });
    });
