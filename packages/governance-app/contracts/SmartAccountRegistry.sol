...
contract SmartAccountRegistry {
    ...
    /**
     * Check Account is valid using modifier.
     *
     * This will also serve as protection against forks, because at the time the chain forks,
     * we can kill the registry, which will then 'invalidate' the accounts that are stored
     * on the old chain.
     */

    /**
     * modifier checkAccount(address account, address registry) return (bool) {
     *  if ( registry.isValidContract(account) != 1 ) {
     *    revert();
     *    _;
     *  }
     * }
    ...
    // We recommend using checkAccount(0x23424234242242) on your contracts.
