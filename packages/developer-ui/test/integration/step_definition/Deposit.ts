...
@binding([PageUtils])
class Deposit {
  private initialETHTotalAmount = "";
  private initialHNTTotalAmount = "";
  ...
  @when(
    /DepositUserA deposit '(\d*)' amount of ETH from his ETH account to HNP account/,
    undefined,
    AVERAGE_TIMEOUT,
  )
  public async whenUserADepositEthereum(amount: string) {
    if (await this.pageUtils.isElementExist(ETHER_ASSET_TOTAL_AMOUNT_ID)) {
      this.initialETHTotalAmount = await this.pageUtils.getElementInnerText(
        ETHER_ASSET_TOTAL_AMOUNT_ID,
      );
    }
  ...
  @when(
    /DepositUserA deposit '(\d*)' amount of HNT from his ETH account to HNP account/,
    undefined,
    AVERAGE_TIMEOUT,
  )
  public async whenUserADepositHyperToken(amount: string) {
    if (
      await this.pageUtils.isElementExist(HYPER_TOKEN_ASSET_TOTAL_AMOUNT_ID)
    ) {
      this.initialHNTTotalAmount = await this.pageUtils.getElementInnerText(
        HYPER_TOKEN_ASSET_TOTAL_AMOUNT_ID,
      );
    }
  ...
  public async thenUserAEtherBalanceIncreases(
    addedETHAmount: string,
    addedHPTAmount: string,
  ) {
    const totalETHAmount = await this.pageUtils.getElementInnerText(
      ETHER_ASSET_TOTAL_AMOUNT_ID,
    );
    const addedETHAmountInWei = utils.parseEther(addedETHAmount).toString();
    const totalETHAmountInWei =
      Number(this.initialETHTotalAmount) + Number(addedETHAmountInWei);

    expect(Number(totalETHAmount)).toBe(totalETHAmountInWei);

    const totalHPTAmount = await this.pageUtils.getElementInnerText(
      HYPER_TOKEN_ASSET_TOTAL_AMOUNT_ID,
    );
    const addedHPTAmountInWei = utils.parseEther(addedHPTAmount).toString();
    const totalHPTAmountInWei =
      Number(this.initialHNTTotalAmount) + Number(addedHPTAmountInWei);
