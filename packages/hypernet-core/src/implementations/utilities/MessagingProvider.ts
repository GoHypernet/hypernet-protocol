...
import { IBrowserNodeProvider, IConfigProvider, IConfigProviderType, IMessagingProvider, IBrowserNodeProviderType, IContextProviderType, IContextProvider } from "@interfaces/utilities";
...
import { IAjaxUtils, IAjaxUtilsType } from "@web-integration/../../utils/src/IAjaxUtils";
import { ResultUtils } from "@hypernetlabs/utils";

@injectable()
export class MessagingProvider implements IMessagingProvider {
    protected initializeResult: ResultAsync<IBasicMessaging, MessagingError> | undefined;

    constructor(@inject(IConfigProviderType) protected configProvider: IConfigProvider,
        @inject(IContextProviderType) protected contextProvider: IContextProvider,
        @inject(IBrowserNodeProviderType) protected browserNodeProvider: IBrowserNodeProvider,
    ...
    public getBasicMessaging(): ResultAsync<IBasicMessaging, MessagingError> {
        ...
        this.initializeResult = ResultUtils.combine([this.configProvider.getConfig(),
        this.getBearerToken()])
            .andThen((vals) => {
                ...
                return ResultAsync.fromPromise(basicMessaging.connect(),
                    (e) => { return new MessagingError("Unable to connect to NATS", e); })
                    .map(() => {
